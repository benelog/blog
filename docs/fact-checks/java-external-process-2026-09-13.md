...
- 이전 기록: [2026-09-06 검증](java-external-process.md)
...
- 이번 작업은 검증과 수정 제안이다. 본문과 예제 소스, 이전 검증 기록은 변경하지 않았다. 아래 행 번호는 위 커밋 기준이다.
...
벤치마크의 과거 절대 수치, 원래 서버의 장애 원인, helper 교체 실험, JDK 27 바이너리 실행은 이번에 재현하지 않았다. 벤치마크 코드는 측정 구간과 해석을 검토했으며 이전 검증의 한계가 그대로 적용된다. 본문·예제·사이트 설정을 바꾸지 않았으므로 사이트 빌드는 실행하지 않았다.
