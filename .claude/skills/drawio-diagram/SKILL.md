---
name: drawio-diagram
description: draw.io(.drawio) 다이어그램을 작성하고 PNG로 내보내 블로그 글에 넣는 워크플로. 다이어그램·도식·구조도를 그리거나 아키텍처·파이프라인·흐름을 시각화해 달라는 요청, .drawio 파일의 생성·수정·PNG 내보내기, 글에 다이어그램을 넣는 작업에는 draw.io를 명시하지 않아도 반드시 이 스킬을 사용한다.
---
