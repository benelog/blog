...
* 2019.04.14
** 테스트한 코드를 https://github.com/benelog/blog/tree/master/examples/beanutils-test 로 올림
* 2010.06.21
...
* Spring(property descriptor)  : Spring의 BeanUtils.getPropertyDescriptors를 사용. Spring의 BeanUtils에는 직접적으로 Map에서 Bean으로 복사해주는 메서드가 없어서 이 방식을 이용했습니다.
...
* Apache Commons(populate) : Apache comons BeanUtils의 BeanUtils.populate 메서드를 사용
...
아뭏든 위의 결과를 봐서는 되도록 성능이 민감한 곳에는 직접 setter를 호출해서 복사를 하고, BeanUtils.populate의 다양한 기능이 필요하지 않다면 Spring의 BeanUtils나 Apache commons PropertyUtils를 통해 캐시된 PropertyDescriptor를 정보를 통해 Bean에 접근하는 것이 성능에는 유리하다는 것을 알 수 있습니다.


== 소스
https://github.com/benelog/blog/tree/master/examples/beanutils-test
