...
== IEEE 754 double의 비트 구조와 0.1의 저장 과정

IEEE 754는 실수를 2진수 과학적 표기법인 ±1.가수 × 2^지수^ 꼴로 바꿔서 저장합니다.
`double` (binary64)은 아래 그림처럼 64비트를 부호 1비트, 지수 11비트, 가수 52비트로 나눠 씁니다.

image::img/floating-point/ieee754-double.png[IEEE 754 double의 비트 구조와 0.1 저장 예,width=800]

* 부호(s): 0이면 양수, 1이면 음수입니다.
* 지수(e): 실제 지수에 1023을 더한 값을 저장합니다. 음수 지수까지 부호 없이 표현하기 위한 방식으로, 저장된 값이 1019라면 실제 지수는 1019 - 1023 = -4입니다.
* 가수(f): 1.f에서 소수점 아래 52비트입니다. 정규화된 수는 정수부가 항상 1이므로 그 1은 저장하지 않고 소수부만 저장합니다.

0.1을 이 구조에 담으려면 먼저 1.가수 × 2^지수^ 꼴로 정규화해야 합니다.
이는 십진수 과학적 표기법과 같은 원리입니다.
십진수 과학적 표기법에서는 소수점을 옮겨 정수부를 1~9 사이 한 자리로 만들고, 옮긴 자릿수를 10의 지수로 나타냅니다.
예를 들어 0.001은 1.0 × 10^-3^ 으로 씁니다.
2진수에서도 마찬가지로 2를 곱하거나 나눠서 정수부가 1이 되도록, 즉 값이 1 이상 2 미만이 되도록 만듭니다.

이제 0.1을 이 꼴로 바꿔 보면 그림 아래쪽처럼 됩니다.
