...
* **Turso로 보내는 질의**: 목적지가 도쿄(일본)라서 아시아 단가가 적용됩니다. 도쿄에서 같은 도쿄로 나가더라도 Google 밖의 주소이므로 인터넷 전송으로 계산됩니다. 다만 나가는 것은 SQL 질의문이라 매우 작고, 결과 행은 들어오는 방향이라 무료입니다.
* **사용자 브라우저로 보내는 응답**: 독자가 대부분 한국에 있으므로 무료 구간 없이 $0.19/GiB 구간에 들어갑니다. 실제로는 이쪽이 전송량의 대부분입니다.

여기서 리전을 바꿔도 단가는 그대로일 가능성이 큽니다.
목적지 기준 과금이라 서울에서 한국 사용자에게 보내도 같은 $0.19/GiB이고, 서울에서 도쿄의 Turso로 보내도 같은 $0.12/GiB입니다.
그래도 Turso로 오가는 양이 예상보다 크면 계산이 달라질 수 있어서, 실제로 운영하면서 청구서에 잡히는 전송량과 금액을 확인한 뒤 리전 구성을 다시 조정할지 검토할 생각입니다.
