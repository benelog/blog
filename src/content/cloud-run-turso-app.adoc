...
[NOTE]
====
Cloud Run 요금 문서는 무료 전송을 "북미 안에서 월 1GiB"로 설명하는데, 네트워킹 요금표에는 위처럼 목적지별로 0~1GiB 무료 구간이 표시됩니다.
두 문서의 설명이 서로 다르므로 무료 1GiB를 기대하기보다, **한국 목적지에는 무료 구간이 없다**는 쪽만 확실한 조건으로 잡는 편이 안전합니다.
====
