...
SQLite 파일 하나 분량의 데이터를 다루는 개인 앱에 맞고, 드라이버(`libsql-client-go`)가 순수 Go라서 Go로 만든 앱을 CGO 없이 빌드하는 구성이 그대로 유지됩니다.
로컬 개발은 SQLite 파일을 쓰고 배포는 Turso를 쓰는데, 같은 `database/sql` 코드가 그대로 돕니다.

함께 쓴 Netlify는 도메인 연결용입니다. 그 선택의 대가는 뒤의 "응답 시간을 좌우한 리전과 프록시" 섹션에서 다룹니다.
...
* **Turso 무료 요금제의 그룹 위치는 나중에 바꿀 수 없습니다.** 처음 만들 때 앱이 뜰 리전에 맞춰야 합니다. 그룹은 1개로 제한되지만 데이터베이스는 100개까지 만들 수 있고, 저장 용량 한도는 5GB입니다.
