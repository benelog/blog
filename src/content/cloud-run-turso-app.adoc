...
메인 HTML도 ``Accept-Encoding``을 붙이든 붙이지 않든 같은 크기로 돌아옵니다.
HTML·CSS·JavaScript는 보통 3~5배까지 줄어드는 자산이므로, 단가가 1.58배 오른 것과 별개로 실제로 나가는 바이트 자체가 늘었습니다.

앱에 gzip 미들웨어를 넣거나 정적 파일을 미리 압축해 함께 embed하면 대부분 회복할 수 있는 문제입니다.
그 전까지는 이 구성이 예전보다 전송 조건이 나쁘다는 점을 감안하고 봐야 합니다.

Turso로 오가는 양이 예상보다 크면 계산이 달라질 수 있어서, 실제로 운영하면서 청구서에 잡히는 전송량과 금액을 확인한 뒤 리전 구성을 다시 조정할지 검토할 생각입니다.
이 전송 조건 때문에 Netlify 프록시로 되돌리는 선택지도 열어 두고 있습니다.
다만 직접 매핑을 실제로 운영하며 확인해 보려고 당분간은 지금 구성을 그대로 두겠습니다.
지금은 측정된 값이 없어서 판단을 미뤄 둡니다.
