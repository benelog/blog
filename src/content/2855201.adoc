...
* https://github.com/benelog/blog/blob/main/examples/btrace-scripts/src/main/java/DbcpMonitorSimple.java[DbcpMonitorSimple.java] : BasicDataSource.getConnection이 한번 호출될 때 BasicDataSource의 모든 정보와 BasicDataSource.connetionPool의 정보를 출력
* https://github.com/benelog/blog/blob/main/examples/btrace-scripts/src/main/java/DbcpMonitor.java[DbcpMonitor.java] :  이벤트 방식은 1과 같고, BasicDataSource의 모든 필드와 BasicDataSource.connetionPool에서 _numActive 값만을 보여줌
* https://github.com/benelog/blog/blob/main/examples/btrace-scripts/src/main/java/DbcpActiveConnectionMonitor.java[DbcpActiveConnectionMonitor.java] : 여러 개의 Datasource의 Active connection 갯수를 구할 때 사용. getConnection에서 URL별로 active connection 갯수를 저장해 두었다가, Btrace에서 이벤트를 날리면 출력을 해주고 종료
...
소스는 https://github.com/benelog/blog/tree/main/examples/btrace-scripts[examples/btrace-scripts]에 올렸습니다.
