...
6. 서버가 올라간 다음에 템플릿의 HomeController 클래스를 수정해보고, 메서드의 내용을 수정한 다음에  아래와 같이 전체 "Realoding..." 메시지가 보이며 applicationContext loading 없이 해당 클래스만 리로딩 되는 것을 확인한다.
...
* Spring MVC의 controller에 @RequestMapping이 달린 새로운 메서드를 추가한 것을 인식 - 잘 안 됨
