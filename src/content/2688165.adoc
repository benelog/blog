...
== Test 메서드와 Import에 대한 Template 설정
...
테스트 메서드를 추가할 때 필요한 library import로 메서드 선언을 한꺼번에 해주는 템플릿이다.이름을 spec으로 지정해서 쓰고 있다.
...
위의 메서드 추가 template에서 해주는 일 중 import를 하는 부분만 수행해준다.
...
== 자동생성되는 메서드에 UnsupportedOperationException 던지기 설정
자동생성 되어서 아직 구현되지 않은 메서드를 test fail로 인식하게 함 ( http://toby.epril.com/?p=706 참고)
