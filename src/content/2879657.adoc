...
2. Maven Jetty plugin : JSP만 고칠 때나 클래스 파일을 고치면 전체 애플리케이션을 reloading을 하는 것이 유리할 때.
...
대부분의 웹애플리케이션은 Servlet Spec만 따라서 개발하므로 Jetty에서 돌아가면 Tomcat에서 거의 돌아간다고 봐도 됩니다. 물론 성능테스트나 프로파일링 같은건 당연히 Tomat을 띄워서 해야겠죠.
