...
봇이 답할 때 참고하는 문서는 `scripts/QMD_repos.yml`에 정의헀습니다.
...
Modal은 실제 사용자 호출을 처리되는 시간동안에만 컨테이너가 깨어나게 하는 서버리스 플랫폼입니다.
대기 시간에는 비용이 발생하지 않고, $5의 무료 크레딧이 매달 주어지므로 트래픽이 띄엄띄엄한 Telegram 챗봇은 0원으로 운영할 수 있을 것으로 예상합니다.
...
Hermes는 자기 발전적인 Agent이기에 실행 중에 자동으로 새로운 skill이 생기거나 기존 skill이 업데이트될 수 있습니다.
agent의 주인인 제가 그 변화를 추적하고 필요하면 직접할 필요성도 있다고 판단했습니다.
