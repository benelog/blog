...
|K-QMD | 검색 엔진 | Markdown, Asciidoc 을 임베딩으로 인덱싱하고 의미 검색을 MCP로 제공. QMD에 Kiwi 형태소 분석을 결합한 fork
|Modal | 실행 환경 | 컨테이너 실행, 데이터 저장 공간
|===

OpenClaw와 Hermes를 비교하는 글에서 Hermes가 클라우드에 올리기 편하다는 설명을 보고 선택했습니다.
검색 엔진은 처음엔 QMD를 썼는데, 한글 문서에서 검색 recall이 낮아서 한글 형태소 분석기를 결합한 link:https://github.com/jylkim/kqmd[K-QMD] fork로 옮겼습니다. K-QMD는 binary 이름과 CLI를 그대로 유지하는 drop-in replacement라 교체 비용이 거의 없었습니다.
...
전체 소스는 link:https://github.com/benelog/hermes-modal[`benelog/hermes-modal`] 저장소에 있습니다. 이 글에서는 그중 설계 의도를 보여주는 코드 조각만 발췌해 설명합니다. Modal로 실행하는 파이썬 앱의 설계와 구현은 Claude Code의 도움을 받았습니다.
...
[source,python]
.link:https://github.com/benelog/hermes-modal/blob/main/modal_app.py[modal_app.py]
----
@app.function(
    image=image,
...
영속적으로 저장되어야할 파일은 Modal의 Volume에 저장했습니다. 이를 이용해서 Hermes 설정 파일, K-QMD 인덱스, 지식 DB 역할의 Git 저장소가 컨테이너 재시작과 무관하게 유지됩니다.
...
봇이 답할 때 참고하는 문서는 link:https://github.com/benelog/hermes-modal/blob/main/scripts/qmd_repos.yml[`scripts/qmd_repos.yml`]에 정의했습니다.

[source,yaml]
.link:https://github.com/benelog/hermes-modal/blob/main/scripts/qmd_repos.yml[qmd_repos.yml]
----
repos:
  - name: blog
...
[source,yaml]
.link:https://github.com/benelog/hermes-modal/blob/main/scripts/hooks/skills-autocommit/HOOK.yaml[HOOK.yaml]
----
...
[source,python]
.link:https://github.com/benelog/hermes-modal/blob/main/scripts/commit_skills.py[commit_skills.py]
----
...
* link:https://github.com/jylkim/kqmd[K-QMD] — QMD에 Kiwi 한글 형태소 분석을 결합한 fork (실제 사용 중)
* link:https://github.com/tobilu/QMD[QMD] — K-QMD의 upstream인 markdown/asciidoc 임베딩 검색 MCP
