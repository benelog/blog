...
다양한 배포판과 버전의 JDK를 명령어 한 줄로 설치하고 OS의 셸에서 사용할 JDK를 쉽게 지정할 수 있게 해주는 도구들을 소개합니다.
...
3. OS의 환경변수를 지정합니다.
** `JAVA_HOME`
*** JDK의 압축을 푼 디렉터리를 지정합니다.
*** Maven이나 Tomcat 같은 솔루션에서 이 환경변수로 JDK의 위치를 참조합니다.
** `PATH`
...
반복적인 작업을 셸스크립트나 배치파일로 할 수도 있습니다.
...
설치된 JDK의 `java` , `javac` 도구는  `/usr/bin/java` , `/usr/bin/javac` 에서 심볼릭 링크로 연결되어 어느 디렉터리에서나 실행될 수 있게 됩니다.
이 심볼릭 링크는 이어서 소개할 update-alternatives / alternatives 도구로 관리할수 있습니다.
`JAVA_HOME` 환경 변수는 직접 `~/.bashrc` 와 같은 셸별 설정 파일에 넣어줘야 합니다.
...
이 심블릭 링크는 `/etc/alternatives/java` 를 중간에 거쳐서 실제 설치한 디렉터리로 연결된 다는 것을 아래와 같이 확인할 수 있습니다.
...
`/usr/bin/java` 보다 더 우선 순위가 높게 먼저 선언된 디렉터리에 `java`가 있다면 `update-alternatives` 에서 지정한 java가 실행되지 않을 수도 있습니다.
SDKMAN, Jabba 등을 함께 사용한다면 이 점을 유의해야 합니다.
현재 셸, 디렉터리에서 어느 `java` 를 실행하고 있는지는 `which java` 로 확인할 수 있습니다.

* 장점
** OS에서 기본적으로 제공하는 도구라서 별도의 설치 과정이 필요하지 않습니다.
** YUM/APT 과 자연스럽게 함께 쓰이는 도구입니다.
* 단점
** 심블릭 링크로 셸에서 사용할 디폴트 버전을 지정하는 기능만 있습니다.
...
위의 명령이 수행되고 나면 `\Program Files\` 디렉터리 아래에 JDK 들이 위치하게 됩니다.
...
여러 배포판을 설치할 경우 `PATH` 환경 변수의 값도 새로 설치한 배포판의 `%JAVA_HOME%\bin` 디렉터리가 뒤 쪽에 계속 추가만 됩니다.
...
=== direnv
https://direnv.net/[direnv] 는 특정 디렉터리와 그 하위 디렉터리에서만 사용할 환경 변수를  지정할 수 있는 도구입니다.
...
이후로 이 파일이 있는 디렉터리에 들어가면 이 환경변수가 활성화됩니다.
cd 명령으로 디렉터리에 들어가면 아래와 같은 메시지가 콘솔에 보입니다.
[source]
----
...
`add` 로 지정한 디렉터리에서 JDK의 버전을 인식하여 아래와 같은 메시지가 나옵니다.
...
해당 셸에서 임시로 사용할 버전은 `shell` 명령으로 지정합니다.
...
현재 디렉터리에서 사용할 버전은 `local` 명령으로 지정합니다.
...
위와 같이 디렉터리에 지정된 버전은 `.java-version` 이라는 파일에 저장됩니다.
다음 번에 같은 디렉터리에서 java를 실행하면 이 파일에 지정된 해당 버전이 선택됩니다.
...
jEnv로 JDK 버전을 지정한 후 `which java` 로 어느 디렉터리에 있는 `java` 와 연결되는지 확인을 해보면
`~/.jenv/shims/java` 가 나옵니다. 이 파일의 내용을 보면 실제 설치한 JDK의 `java` 가 아닌 셸 스크립트라는 것을 알수 있습니다.
...
`~/.jenv/versions/ 디렉터리에 각 버전별로 실제로 JDK가 설처되어있는 디렉터리로의 심볼릭 링크가 들어가 있습니다.

[source]
.`~/.jenv/versions` 디렉터리 안의 심볼릭 링크
----
...
* 장점
** 다양한 범위(디폴트(global), 디렉터리별, 셸 범위)의 버전 방식을 지원합니다.
* 단점
...
현재 셸에서 사용할 버전만 임시로 바꾸고 싶다면 `default` 대신 `use` 명령을 씁니다.
[source]
----
...
* 단점
** 특정 디렉터리에 들어갔을 때 사용할 버전을 자동을 지정하는 기능이 없습니다.
...
현재 셸에서 사용할 버전은 아래와 같이 지정할 수 있습니다.
...
`echo $PATH` 로 PATH 값을 확인해보면, 가장 앞에 설치한 JDK의 bin 디렉터리를 지정할 것을 확인할 수 있습니다.

같은 디렉터리에 `.jabbarc`라는 파일이 있다면, 그 파일에 지정된 버전을 참조할 수 있습니다.
...
다음 번에 같은 디렉터리에 들어왔을 떄에는 `jabba use` 만 간단하게 실행해서 같은 효과를 낼 수 있습니다.
direnv나 jEnv를 쓸 때처럼 디렉터리에 들어가면 자동으로 환경변수를 바꾸어주는 기능은 없습니다.

현재 셸범위의 JDK 버전만 지정한다는 점이 jabba의 장점이나 단점입니다.
...
* 단점
** 디폴트 버전 지정이 없습니다.
** 디렉터리별 버전 비전 기능이 완전 자동이 아닙니다. 해당 디렉터리에서 `jabba use` 를 한번 입력해야 합니다.
...
Amazon Corretto, GraalVM 등 다양한 배포판의 여러버전을 설치해보고 싶다면 SDKMAN이나 jabba를 함께 쓰는 것을 추천합니다.
...
* *(*3)* : jEnv나 direnv처럼 디렉터리에 들어가면 자동으로 특정 JDK 버전이 선택되는 방식은 아니기 때문에 △로 표기했습니다.
...
** 특정 디렉터리에서 사용한 버전을 지정할 때는 direnv를 씁니다.
** 셸에서 일시적으로 사용할 버전을 지정할 때는 SDKMAN, jabba를 씁니다.
