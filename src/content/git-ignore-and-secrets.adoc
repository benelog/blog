...
API 키나 `.env` 파일 같은 비밀을 Git 저장소에서 어떻게 다룰지는 선택지가 여러 갈래입니다. 아예 추적하지 않도록 무시하는 방법, 암호화해서 커밋하는 방법, 실수로 커밋되는 것을 탐지하는 방법이 각각 다른 도구로 발전해 왔습니다. 이 글에서는 Git의 무시 규칙인 `.gitignore`와 `.git/info/exclude`의 차이에서 출발해, 암호화 커밋 도구인 git-secret과 SOPS, 유출 스캐너인 gitleaks까지 정리합니다.
