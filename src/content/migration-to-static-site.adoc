...
새로운 블로그는 정적 사이트 생성기(Static Site Generator)로 Git 저장소에 있는 AsciiDoc 형식의 콘텐츠를 HTML 파일로 만들어서 배포하는 구조입니다.
덕분에 Egloos로부터의 마이그레이션 작업이 더 수월했고 더 좋은 서비스가 있을 때 쉽게 이전을 할 수 있습니다.

콘텐츠 저장소는 GitHub외에도 Gitlab, Bitbucket 등의 대안이 있고, 콘텐츠 빌드 배포 플랫폼도 다양합니다.
필요하다면 더 좋은 기능이 있는 서비스를 유료로 사용할 수도 있습니다.
콘텐츠 편집기도 여러가지를 사용할 수 있습니다.
...
AsciiDoc으로 작성된 포스트의 원본은 https://github.com/benelog/blog/tree/main/src/content[github/benelog/blog/src/content] 에서 이력을 관리합니다.
...
빌드 스크립트는 https://github.com/benelog/blog/blob/main/build.gradle[build.gradle]에서 확인이 가능합니다.
Travis CI 설정인 https://github.com/benelog/blog/blob/main/.travis.yml[.travis.yml]은 아래와 같이 단순합니다.
...
Egloos의 이전 포스트들은 https://blog.benelog.net/904735 와 같이 HTML 확장자가 없게 되어 있습니다.
...
`Find in Path (Ctrl + Shift + F)` , `Replace in Path (Ctrl + Shift + R)` 으로 여러 파일에 걸쳐 문자열을 검색하고 치환하는 기능은 마이그레이션 된 콘텐츠를 정리할 때 많은 도움이 되었습니다.
...
=== 마이그레이션 프로그램
마이그레이션 프로그램의 소스와 사용법은 https://github.com/benelog/egloos-migration/ 을 참고하시기 바랍니다.
이것도 제가 알고 있는 기술들을 활용해서 Kotlin + Spring 으로 작성했습니다.

콘텐츠를 담은 파일을 생성할 때 Kotlin의 문자열 문법이 유용할 것으로 예상해서 언어는 Kotlin을 선택했습니다. 아래와 같은 코드입니다.
