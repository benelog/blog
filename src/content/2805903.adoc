...
Spring AOP에서 동일한 target클래스에 결합되는 Aspect의 우선순위는  http://static.springsource.org/spring/docs/3.0.x/api/org/springframework/core/Ordered.html[Ordered] 인터페이스를 구현하거나 @Order 애너테이션으로 지정합니다.
...
OrderOneAspect 은 애너테이션으로, OrderTwoAspect은 인터페이스로 order 값을 선언했습니다.
