...
이 글에서는 이 문제를 해결한 설정을 공유합니다.
Ubuntu 24.04 + zsh + git 2.43 + scm_breeze 환경에서 확인했습니다.

== delta 설치와 설정

https://github.com/dandavison/delta[delta]는 git의 pager를 대체하는 도구입니다. pager는 `git diff`나 `git log`의 긴 출력을 한 화면씩 넘겨 보여주는 프로그램으로, 기본값은 `less`입니다. delta는 이 자리를 대신하면서 구문 강조, 줄 번호, 그리고 **줄 안에서 바뀐 부분만 강조하는 기능**을 제공합니다.
