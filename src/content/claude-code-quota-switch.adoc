...
이 글은 Claude Code에게 프롬프트를 내려서 hook 스크립트를 생성하고, 생성된 코드를 분석하는 방식으로 구성됩니다. 생성된 코드에서 발견한 버그와 수정 사항도 함께 다룹니다.

=== 전제 조건

* Claude Code v2.1.78(2026년 3월 18일 릴리즈) 이상 (`StopFailure` hook 이벤트 지원)
* Linux 환경 (GNU `date`, `grep -P` 사용). macOS에서의 차이점은 <<macos>> 참고
* `jq` 설치 필요
...
2. 초기화 시간 자동 기록: on-rate-limit.sh에서 transcript 파일을 읽어 "resets TIME (TIMEZONE)" 패턴을 파싱하여 초기화 시간을 ~/.claude/quota-switch-reset-time에 epoch으로 저장해줘.
3. 초기화 시간 수동 기록(fallback): 자동 파싱이 실패할 경우를 대비해 record-quota-reset.sh도 만들어줘. "4h", "30m" 같은 상대 시간과 절대 시간을 지원해야해.
4. 자동 복귀: SessionStart hook에서 check-quota-switch.sh를 실행해서, 현재 시간이 초기화 시간을 지났으면 ~/.claude/.credentials.json을 삭제하여 재로그인을 유도해줘. startup과 clear 두 matcher 모두 등록해줘.
____

Claude Code는 이 요청을 세 개의 스크립트와 `settings.json`의 hook 설정으로 구현했습니다.

== 생성된 코드 분석

=== Quota 소진 감지 및 초기화 시간 자동 기록

`~/.claude/scripts/on-rate-limit.sh` 는 `StopFailure` hook에서 실행되는 스크립트입니다. Quota 소진 시 transcript에서 초기화 시간을 자동으로 파싱하여 기록합니다.
...
# 2. Search transcript for reset time
# 3. If found, auto-record the reset time
...
# Step 2: Extract transcript_path from hook input
TRANSCRIPT=$(echo "$INPUT" | jq -r '.transcript_path // empty')
if [ -z "$TRANSCRIPT" ] || [ ! -f "$TRANSCRIPT" ]; then
    echo "No transcript found" >> "$LOG_FILE"
    exit 0
fi

# Step 3: Search transcript for reset time patterns
# "resets 2pm (Asia/Seoul)", "resets Feb 20, 5pm (Asia/Seoul)" etc.
RESET_LINE=$(grep -oP 'resets?\s+(at\s+)?(\w+\s+\d+,?\s+)?\d{1,2}(:\d{2})?\s*[ap]m(\s*\([^)]+\))?' \
    "$TRANSCRIPT" | tail -1)

if [ -z "$RESET_LINE" ]; then
    echo "No reset time found in transcript" >> "$LOG_FILE"
    exit 0
fi

echo "Found reset info: $RESET_LINE" >> "$LOG_FILE"

# Step 4: Parse the reset time
...
`StopFailure` 이벤트의 JSON 입력에는 `transcript_path` 가 포함되어 있습니다. Transcript는 Claude Code 세션의 모든 대화 내용이 기록된 JSONL 파일입니다. 이 스크립트는 transcript에서 `resets TIME (TIMEZONE)` 패턴을 grep하여 초기화 시간을 추출합니다.

실제 transcript 파일을 확인한 결과, quota 소진 메시지는 다음과 같은 형식으로 기록되어 있었습니다:

[source,json]
----
{"content":[{"type":"text","text":"You've hit your limit · resets 11am (Asia/Seoul)"}],"error":"rate_limit",...}
----

`content[].text` 필드에 초기화 시간이 포함되어 있고, 같은 JSON 줄에 `"error":"rate_limit"` 도 함께 기록됩니다. 이 형식이 유지되는 한 위 스크립트의 grep 패턴으로 파싱할 수 있습니다.
...
`StopFailure` 이벤트는 API 에러로 턴이 종료될 때 발생하며, `"matcher": "rate_limit"` 으로 quota 소진만 필터링합니다. `SessionStart` 이벤트는 `startup` (새 세션 시작)과 `clear` (`/clear` 실행) 두 경우 모두 같은 스크립트를 실행하도록 설정했습니다.

== 사용 방법

=== 1단계: quota A 소진

Quota A가 소진되면 `StopFailure` hook이 자동으로 `on-rate-limit.sh`를 실행합니다. Transcript에서 초기화 시간을 파싱하여 `~/.claude/quota-switch-reset-time`에 기록합니다. `~/.claude/hook-events.log`에서 "Auto-recorded quota reset time"이 기록되었는지 확인할 수 있습니다.

자동 파싱에 실패한 경우, 로그에 실패 사유가 기록됩니다. 이때는 수동으로 기록합니다:
...
== 배경: Quota 소진 시 Claude Code의 동작

=== 사용자에게 보여주는 메시지

Pro plan이나 Max plan의 경우 quota 소진 시 Claude Code가 초기화 예정 시간을 포함한 메시지를 안내합니다.
...
초기화 시간은 이 JSON 필드에 직접 포함되지 않습니다 (https://github.com/anthropics/claude-code/issues/36056[#36056], https://github.com/anthropics/claude-code/issues/30784[#30784]에서 기능 요청 중). `on-rate-limit.sh`에서 `transcript_path`의 파일을 grep하여 초기화 시간을 추출하는 이유입니다.
...
* **Transcript 파싱의 메시지 형식 의존**: `on-rate-limit.sh`의 transcript 파싱은 `resets TIME (TIMEZONE)` 패턴에 의존합니다. Claude Code 버전에 따라 메시지 형식이 달라져 왔으므로, 향후 형식이 변경되면 파싱이 실패할 수 있습니다. 실패 시 `~/.claude/hook-events.log` 에 원인이 기록되며, `record-quota-reset.sh`로 수동 기록할 수 있습니다.
...
* macOS에서 Claude Code의 credentials는 Keychain에 저장될 수 있어, `~/.claude/.credentials.json` 삭제 방식이 동작하지 않을 수 있습니다.
