...
주의할 점이 하나 있습니다. 생성자에 ``double``을 넘기면 오차도 그대로 따라옵니다.
...
예를 들어 2.5를 정수로 만들면 `HALF_UP`은 3, `HALF_EVEN`은 2, `DOWN`은 2가 됩니다.
...
지연 시간에 민감한 금융 시스템 중에는 `BigDecimal` 없이 만든 것도 많이 돌아가고 있다는 점도 근거로 듭니다.
...
또 `long` 방식은 기본 산술 연산자로 계산하므로, `calculate()` 예제의 ``amount * rate * 0.01``처럼 중간에 부동소수점이 섞여도 컴파일러가 막지 않습니다.
