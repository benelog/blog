...
IEEE 754는 하나의 형식이 아니라 여러 부동소수점 형식을 정의하는 표준입니다.
...
다음 절에서는 자바에서 쓸 수 있는 대안을 살펴봅니다.

== 자바에서 소수점 연산 오차를 피하는 대안

`double` 형의 연산에서 생길 수 있는 오차를 피할 수 있는 3가지 방안들을 소개합니다.
...
1을 3으로 나누는 것처럼 십진수로 끝나지 않는 나눗셈은 자릿수와 반올림 방식을 지정하지 않으면 예외가 납니다.
...
소수점 이하 몇 자리까지 어떤 방식으로 반올림할지는 여전히 팀이 정해서 지켜야 하는 규칙으로 남습니다.
...
다만 `amount * rate * 0.01` 곱셈은 부동소수점 연산으로 수행됩니다.
`long` 값이 `float` 로 변환되면서 정밀도를 잃기 때문에, 입력과 리턴 타입이 정수여도 금액이 커지면 결과가 어긋납니다.
...
정수형 최소 단위 방식이 오차를 없애는 것은 이렇게 모든 중간값을 정수나 `BigDecimal` 로 유지할 때뿐입니다.
금액이 대표적인 사례일 뿐, 소수점 이하 자릿수가 정해진 값이라면 무엇이든 같은 방식으로 정수로 환산해 다룰 수 있습니다.
...
== 수치 표현과 도메인 모델, 두 축의 선택 기준

세 방법 모두 중간 계산까지 부동소수점을 쓰지 않는다면 오차를 피할 수 있지만, 각각의 추가 비용이 있습니다.
그리고 세 방법이 같은 층위에 있는 것도 아닙니다.
`BigDecimal` 과 정수형 최소 단위는 수를 어떤 형태로 담을지에 대한 선택이고, Java Money는 그 위에서 금액과 통화를 어떤 타입으로 표현할지에 대한 선택입니다.
`Money` 가 내부적으로 ``BigDecimal``을, `FastMoney` 가 `long` 을 쓰는 것이 그 관계를 보여줍니다.
그래서 선택도 수치 표현과 도메인 모델의 두 단계로 나누어 하게 됩니다.

``BigDecimal``는 표현력과 시스템 자원 사용량에서 단점이 생깁니다.
...
값이 `long` 하나이므로 객체를 만들지 않고, 연산도 CPU의 정수 연산으로 끝납니다.
대신 개발자가 정확한 연산을 늘 의식해야 합니다.
최소 단위를 무엇으로 볼지를 시스템 전체가 같은 규칙으로 지켜야 하고, 나눗셈이나 비율 계산에서는 어느 시점에 어떤 `RoundingMode` 로 반올림할지를 매번 직접 정해야 합니다.
앞에서 본 `calculate()` 예제의 `amount * rate * 0.01` 처럼 중간 계산 한 곳만 부동소수점에 맡겨도 오차가 생깁니다.
금액과 통화를 함께 표현하는 장치도 없어서, `long` 값 하나만 보고는 그것이 원인지 센트인지 알 수 없습니다.

Java Money는 정확성보다는 돈을 다룰 때 편의성과 표현력을 얻는 선택입니다.
`Money` 는 내부적으로 ``BigDecimal``로 계산하므로 위에 정리한 `BigDecimal` 의 비용을 그대로 지고, `MonetaryAmount` 로 한 겹 감싸는 비용이 더해집니다.
그 대신 금액과 통화가 한 값으로 묶여서, 통화가 다른 금액을 더하는 실수를 API가 걸러줍니다.
속도가 문제가 되면 같은 `MonetaryAmount` 인터페이스를 구현한 https://github.com/JavaMoney/jsr354-ri/blob/master/moneta-core/src/main/java/org/javamoney/moneta/FastMoney.java[FastMoney]로 바꿔 끼울 수 있습니다.
`FastMoney` 는 금액을 `long` 하나에 최소 단위로 저장하는 구현으로, 소수점 아래 5자리로 스케일이 고정된다는 제약을 받는 대신 자바독에서는 ``Money``보다 10~15배 빠른 것으로 소개합니다.
앞의 두 방식을 같은 API 아래에서 바꿔 쓸 수 있는 셈입니다.
다만 구현을 바꾸면 수치 표현도 함께 바뀝니다.
`FastMoney` 는 스케일이 5자리로 고정되고 값의 범위도 `long` 에 갇히므로, 기존 계산의 정밀도와 반올림 결과가 달라지지 않는지 확인해야 합니다.
JDK에 포함되지 않은 별도 의존성이라는 점도 그대로 남습니다.

먼저 수를 어떤 형태로 담을지 고릅니다.
...
|메모리와 가비지, 연산 속도, 메서드 체이닝 문법

|정수형 최소 단위
|`long`
|객체 생성 없이 가장 빠름
|최소 단위 규칙과 반올림 통제를 개발자가 부담
|===

그다음 그 값을 도메인 타입으로 감쌀지 정합니다.
...
|원시 값 그대로 (`long`, `BigDecimal`)
|추가 의존성이 없고 구조가 단순함
|금액과 통화가 분리되어, 값만 보고는 단위를 알 수 없음

|Java Money (`MonetaryAmount`)
|금액과 통화를 한 값으로 묶어 통화 불일치를 걸러냄
|래핑 비용과 별도 의존성, 구현 선택이 곧 수치 표현 선택
|===

저는 돈과 같이 정밀한 계산이 필요한 영역에서는 최소한 ``double``보다는 ``BigDecimal``을 권장해야 한다고 생각합니다.
`double` 로 금액을 다루려면 오차가 생기는 지점과 반올림할 시점을 모든 계산 경로에서 통제해야 하는데, 한 곳만 놓쳐도 정밀도가 떨어지는 값이 나옵니다.
성능이 실제 병목으로 확인된 상황이라면 `double` 보다는 정수형 최소 단위 계산이 먼저 검토할 대안입니다.
금액 자체는 오차 없이 유지하면서 속도를 얻을 수 있기 때문입니다.
다만 이 방식도 최소 단위를 무엇으로 할지와 반올림 규칙을 팀의 약속으로 유지해야 합니다.
조직에는 늘 새로운 사람이 들어오기 때문에, 모든 사람이 그 약속과 이 글에서 다룬 오차의 특성을 이해하고 있다고 전제하기는 어렵습니다.
따라서 정밀한 연산이 필요한 소수 처리를 하는 코드에서는 팀과 프로젝트의 표준으로 `BigDecimal` 사용을 전수하는 편이 더 안전합니다.
물론 `BigDecimal` 을 쓴다고 반올림 규칙까지 없어지지는 않습니다.
앞에서 봤듯이 나눗셈의 자릿수와 반올림 방식은 여전히 합의해야 합니다.
`BigDecimal` 이 확실히 없애 주는 것은 2진수 표현에서 오는 오차이고, 그만큼 팀이 신경 쓸 규칙이 줄어듭니다.

도메인 모델 축은 통화를 여러 개 다루는지에 따라 갈린다고 봅니다.
통화가 하나뿐이라면 원시 값에 `BigDecimal` 로도 충분하고, 여러 통화가 섞이는 시스템이라면 통화 불일치를 API가 걸러 준다는 이점이 별도 의존성을 감수할 만합니다.
