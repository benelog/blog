...
https://m.yeongnam.com/view.php?key=20110726.010061333590001[영남일보 기사]는 그 오차의 모양을 더 구체적으로 전합니다.
나이스는 동점자의 석차를 가리기 위해 총점을 소수점 이하 16자리까지 표시하도록 짜여 있었는데, 그 아래 자리에서 불규칙적으로 '1'이 나타나는 값이 발생했다는 것입니다.
글 앞부분에서 1.03 - 0.42의 결과로 0.6100000000000001이 나온 것과 같은 모양입니다.

교육과학기술부 점검단의 결과는 어떤 타입에서 오차가 났는지까지 밝혔습니다.
https://www.etnews.com/201109030016[전자신문 기사]는 점검 결과를 "새로 설치된 데이터베이스(DB2)의 특성상 나타나는 실수형(Double형) 자료 연산 오류를 예측하지 못하여 성적 처리 오류가 발생했던 것을 확인했다"고 옮깁니다.
같은 점검 결과를 다룬 https://www.yna.co.kr/view/AKR20110902079600004[연합뉴스 기사]는 동점자 처리의 '오류 보정 코드'를 일부 프로그램에 적용하지 않았고, 연산 조건문에 AND 조건을 OR 조건으로 잘못 입력한 점도 함께 지목합니다.
조건문 오류는 부동소수점과 무관한 별개 원인입니다.
그러나 성적 계산 자체는 ``double``에 해당하는 타입으로 이루어졌고, 그 오차를 보정하는 코드가 빠진 곳에서 석차가 뒤바뀌었습니다.

당시 네트워크 개발자 이경문 씨는 https://www.dailysecu.com/news/articleView.html?idxno=319[데일리시큐 기사]에서 "정확한 수치 표현을 해야 하는 곳에서 부동소수점을 이용한 것"이 문제라며, 오차를 보정할 것이 아니라 정수로 처리하도록 설계를 바꾸거나 임의 정밀도 타입을 써야 한다고 지적했습니다.
이 글 뒷부분에서 소개하는 정수형 최소 단위와 ``BigDecimal``이 바로 그 두 방향입니다.
DB2의 DOUBLE 타입도 IEEE 754 binary64를 따르므로, 성적처럼 소수점 계산이 들어가는 값을 부동소수점으로 다루면 동점 여부 판정 같은 비교 연산이 어긋날 수 있음을 보여 주는 사례입니다.
