...
여기서 반올림 모드는 `java.math.RoundingMode` 로, 0.5를 올릴지(`HALF_UP`) 짝수 쪽으로 붙일지(`HALF_EVEN`) 무조건 버릴지(`DOWN`) 같은 반올림 방식을 고르는 열거형입니다.
...
세 방법 모두 부동소수점 오차를 피할 수 있지만, 각각의 추가 비용이 있습니다.
...
다만 JDK에 포함되지 않은 별도 의존성이라는 점은 그대로 남습니다.
...
|Java Money
|`Money` 는 `BigDecimal`, `FastMoney` 는 `long`
|금액과 통화를 한 값으로 묶어 통화 불일치를 걸러냄
|택한 구현의 비용에 래핑 비용이 더해지고, 별도 의존성이 필요
|===
