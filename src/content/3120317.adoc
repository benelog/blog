...
(자세한 설명은 http://helloworld.naver.com/helloworld/645609[Java의 날짜와 시간 API],전체 소스는 https://github.com/benelog/blog/blob/master/examples/java-date-time/jdk7-date-time/src/test/java/com/nbp/weblab/date/OldJdkDateTest.java[OldJdkDateTest.java)] 참조)
...
== 시간대 변경시점의 오류
처음에 보낸 패치에는 포함되지 못했지만 섬머타임 외에도 우리나라 시간대 변경에 대한 의문도 있었습니다. "yyyy.MM.dd HH:mm (Z)"을 포맷으로 해서, 1954년, 1961년, 1968년의 특정시간과 그 때와 UTC와의 차이를 출력해보면, 아래와 같이 나옵니다. (소스는 https://github.com/benelog/blog/blob/master/examples/java-date-time/jdk7-date-time/src/main/java/com/nbp/weblab/date/TimeZoneChangePoint.java[TimeZoneChangePoint.java] 참조 )
