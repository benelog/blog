...
image::img/habit-chain/dolthub-commit-log.png[DoltHub의 Commit Log 화면. 체크 추가·삭제와 습관 수정이 SQL 문장 단위의 커밋으로 나열되어 있다]
...
* `checks` 표의 PK인 ``(habit_id, check_date)``가 하루에 체크 하나라는 규칙을 스키마에 담고 있습니다. UTC 시각으로 저장하면 하루의 경계가 조회하는 시간대에 따라 달라지므로, 이 규칙을 DB 차원에서 표현할 수 없습니다. 체크 해제도 지금은 날짜 단위 ``DELETE`` 한 문장인데, 시각 저장이면 범위 질의가 됩니다.
...
실행할 DDL은 순번이 있는 목록으로 관리합니다. 목록은 뒤에 붙기만 하고, 순번이 의존 순서를 정합니다. 실행 시점에는 적용되지 않은 문장만 순서대로 골라냅니다.
...
* **버전 표를 두지 않습니다.** 마이그레이션 도구는 `schema_migrations` 같은 표에 적용된 버전을 기록합니다. 이 구성에서는 사용자가 DoltHub 웹 화면에서 스키마를 직접 만질 수 있고, write 엔드포인트에 트랜잭션이 없어 기록이 실제와 어긋날 수 있습니다. 그래서 적용 여부는 마이그레이션마다 실제 스키마를 조사한 결과(`applied`)로 판정합니다. 실제 스키마가 유일한 기록입니다.
* **CREATE 문장은 현재 스키마의 스냅숏입니다.** 도구는 새 DB에도 과거 ``ALTER``를 순서대로 재생하지만, 여기서는 문장 하나가 커밋 하나이고 1.5초에서 2초가 걸려서 재생할수록 첫 준비가 느려집니다. 새 표는 최신 모양으로 바로 만들고, 그 마이그레이션이 뒤의 ``ALTER``까지 덮은 것으로 판정합니다. 3번의 ``applied``가 "habits 표가 없으면 적용된 것"으로 읽히는 이유입니다.
