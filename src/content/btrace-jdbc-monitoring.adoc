...
이 스크립트가 하던 "특정 메소드의 호출 횟수와 평균 실행 시간 집계"는 JDK 25부터 외부 도구 없이 JFR(Java Flight Recorder)로 할 수 있습니다. https://openjdk.org/jeps/520[JEP 520: JFR Method Timing & Tracing]이 `jdk.MethodTiming`, `jdk.MethodTrace` 이벤트를 추가했습니다.
...
다만 JEP 520은 실행 시간만 기록할 뿐 메소드의 파라미터, 리턴값, 필드값은 보여주지 않습니다. link:2855201.html[이전 글]의 DBCP 스크립트처럼 객체 내부 값을 들여다보는 용도라면 여전히 BTrace나 https://github.com/alibaba/arthas[Arthas](`watch`, `monitor`, `trace` 명령) 같은 계측 도구가 필요합니다.

=== 그 밖의 대안

* 특정 메소드가 아닌 전체적인 프로파일링이라면 JFR 상시 기록이나 https://github.com/async-profiler/async-profiler[async-profiler]가 우선 선택지입니다.
