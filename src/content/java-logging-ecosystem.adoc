...
Java 로깅 라이브러리 생태계는 꽤나 복잡해서 라이브러리 간의 관계를 제대로 이해하지 못 하고 사용하는 경우도 많습니다.
SLF4J, Apache Commons Logging, Logback, Log4j 등 많이 쓰이는 라이브러리만도 해도 여러 가지이고, 대부분의 실무에서는 적어도 2개는 조합해서 쓰고 있기 때문입니다.
