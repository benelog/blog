...
앞의 ``reassign`` 예제를 이 구조에 대입하면 다음 그림과 같습니다. ``reassign``은 static 메서드이므로 파라미터 ``tv``는 ``reassign`` 프레임의 0번 지역 변수입니다. javac가 ``main`` 프레임의 1번 지역 변수에 배정한 ``myTv``에 담긴 참조값이 이 칸으로 복사됩니다. 파라미터가 0번부터 들어가는 것은 JVMS가 정하지만, 파라미터가 아닌 지역 변수를 몇 번 칸에 둘지는 컴파일러가 정합니다. 바이트코드 수준에서는 ``aload_1`` 명령이 이 참조값을 ``main`` 프레임의 operand stack에 올리고, https://docs.oracle.com/en/java/javase/26/docs/specs/jvms/jvms-6.html#jvms-6.5.invokestatic[`invokestatic`] 명령이 그 값을 꺼내 새 프레임의 0번 지역 변수에 넣습니다. 두 칸은 서로 다른 프레임에 속한 별개의 저장 공간이므로 ``tv``에 새 객체를 대입해도 ``main`` 프레임의 ``myTv``는 여전히 처음의 객체를 가리킵니다.
...
<2> ``append``는 길이가 4인 새 슬라이스 값을 돌려주고 그 값은 지역 변수 ``s``에만 대입됩니다. 호출한 쪽의 ``nums``는 길이가 3인 원래 슬라이스 값 그대로입니다. 용량이 남아 있어 새 요소가 같은 배열에 쓰이더라도 ``nums``의 길이는 3 그대로이므로 ``nums[3]``은 범위를 벗어납니다. ``nums[:4]``처럼 용량까지 다시 슬라이싱해야 그 요소가 보입니다.
