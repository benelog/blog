...
이 정의는 프로그래밍 언어론의 표준적인 설명을 따릅니다. Christopher Strachey는 1967년 강의를 정리한 https://www.cs.cmu.edu/~crary/819-f09/Strachey67.pdf[Fundamental Concepts in Programming Languages] 3.4.2절에서 파라미터에 표현식의 R-value를 넘기는 방식을 call by value, L-value를 넘기는 방식을 call by reference라고 불렀습니다. 컴파일러 교과서로 널리 쓰이는 **Compilers: Principles, Techniques, and Tools** 2판 1.6.6절도 call by value는 인자를 평가하거나 복사한 값을 파라미터의 저장 위치에 넣는 방식, call by reference는 인자의 주소를 파라미터의 값으로 넘겨 파라미터의 변경이 인자의 변경으로 나타나는 방식이라고 설명합니다.
...
public class ReassignTest {
    ...
    static void reassign(Tv tv) {
        tv = new Tv(); // <2>
        tv.setChannel("Rock");
    }
}
----
<1> main 메서드의 ``myTv``에 담긴 참조값이 복사되어 reassign 메서드의 파라미터 ``tv``에 대입됩니다. 두 변수는 같은 객체를 가리키지만 서로 다른 변수입니다.
<2> 파라미터 ``tv``에 새 객체를 대입합니다. 파라미터 ``tv``만 새 객체를 가리키게 되고, main 메서드의 ``myTv``는 원래 객체를 그대로 가리킵니다. 그래서 출력은 ``Classic``입니다.
...
파라미터 재할당은 호출자 변수에 반영되지 않는데 객체 상태 변경은 호출자 변수로 접근해도 확인되니, "객체는 call by reference로 전달된다"는 설명이 그럴듯하게 들립니다.
...
**Head First Java**에서는 객체 참조를 TV 리모컨에 비유해서 설명합니다. 객체를 전달하면 TV(객체)가 복사되는 것이 아니라 그 TV를 조작할 수 있는 리모컨(참조값)이 복사됩니다. 복사된 리모컨으로 채널을 바꾸면(객체 상태 변경) 원래 리모컨을 가진 사람에게도 바뀐 채널이 보입니다. 복사된 리모컨이 다른 새로운 TV에 연결된 이후에는(재할당, `tv = new Tv()`) 처음의 TV에 영향을 미치지 못하고 원래 리모컨은 여전히 처음의 TV를 참조합니다.
...
다만 이는 언어 수준의 의미를 설명하는 표현이지, 참조값이 원시 메모리 주소와 같은 형식이라고 보장하는 문장은 아닙니다. https://docs.oracle.com/en/java/javase/26/docs/specs/jvms/jvms-2.html#jvms-2.7[Java Virtual Machine Specification 2.7]은 객체의 내부 구조를 특정하지 않으며, Oracle의 일부 구현에서는 참조가 객체 데이터가 아니라 핸들을 가리킬 수도 있다고 설명합니다. 여기서 핸들은 참조와 객체 데이터 사이에 놓인 작은 중간 구조입니다. 참조는 핸들을 가리키고, 핸들 안의 포인터가 다시 객체 데이터와 메서드 테이블을 가리킵니다. 참조값이 객체가 놓인 메모리 주소 그 자체가 아닐 수도 있다는 뜻입니다.
...
앞의 ``reassign`` 예제를 이 구조에 대입하면 다음 그림과 같습니다. ``reassign``은 static 메서드이므로 파라미터 ``tv``는 ``reassign`` 프레임의 0번 지역 변수입니다. ``main`` 프레임의 1번 지역 변수인 ``myTv``에 담긴 참조값이 이 칸으로 복사됩니다. 바이트코드 수준에서는 ``aload_1`` 명령이 이 참조값을 ``main`` 프레임의 operand stack에 올리고, https://docs.oracle.com/en/java/javase/26/docs/specs/jvms/jvms-6.html#jvms-6.5.invokestatic[`invokestatic`] 명령이 그 값을 꺼내 새 프레임의 0번 지역 변수에 넣습니다. 두 칸은 서로 다른 프레임에 속한 별개의 저장 공간이므로 ``tv``에 새 객체를 대입해도 ``main`` 프레임의 ``myTv``는 여전히 처음의 객체를 가리킵니다.

image::img/call-by-value/java-stack-frames.png[reassign 메서드의 프레임과 main 메서드의 프레임이 각자의 지역 변수 배열을 가지고 참조값만 복사되는 구조,width=800]

다만 JVMS는 추상 기계를 정의할 뿐입니다. JVMS 2장의 서두는 런타임 데이터 영역의 메모리 배치를 구현자의 재량에 맡긴다고 밝힙니다. 예를 들어 JIT 컴파일러가 만든 기계어 코드는 파라미터를 CPU 레지스터에 둘 수도 있습니다. 그렇더라도 호출한 쪽의 변수와 파라미터가 별개의 변수라는 언어의 의미는 달라지지 않습니다.
...
재할당이 막혀 있을 뿐 나머지는 Java와 같습니다. JVM에서 실행할 때 전달되는 것은 Java와 같은 참조값의 복사본입니다. 파라미터가 final이라는 규칙은 언어 사양이므로 Kotlin/JS나 Kotlin/Native로 컴파일해도 달라지지 않습니다.
...
----
<1> 파라미터 ``tv``에 새 객체를 대입해도 호출한 쪽의 ``myTv``는 바뀌지 않습니다.
...
진짜 call by reference가 어떤 것인지는 {cpp}의 참조 파라미터와 비교하면 분명해집니다. {cpp}은 C의 포인터에 더해 참조 타입을 제공합니다.
...
{cpp}에는 C에서 물려받은 포인터 파라미터도 있어서 둘을 구분해야 합니다. 포인터 파라미터는 {cpp}에서도 call by value입니다. ``void swap(int* x, int* y)``처럼 포인터를 받으면 앞의 C 예제와 똑같이 포인터 값의 복사본이 파라미터에 대입됩니다. 함수 안에서 ``x``에 다른 주소를 대입해도 호출한 쪽 변수에는 반영되지 않습니다.
...
Python도 Java와 같은 구조입니다. 공식 튜토리얼 https://docs.python.org/3/tutorial/controlflow.html#defining-functions[4.8 Defining Functions]는 인자가 호출된 함수의 지역 심벌 테이블에 도입되므로 값이 항상 객체 참조인 call by value로 전달된다고 설명합니다.
...
<2> 파라미터 ``tv``를 새 객체에 다시 묶으므로 호출한 쪽의 ``my_tv``는 그대로이고, ``Classic``이 출력됩니다.
<3> ``tv``가 가리키는 객체의 속성을 바꾸므로 호출한 쪽의 ``my_tv``로 접근해도 바뀌어 있고, ``Rock``이 출력됩니다.

Java의 `Tv` 예제, JavaScript의 ``reassign``과 `changeState` 예제와 같은 구분입니다.

정수나 문자열을 넘기면 호출한 쪽 변수가 그대로인데 리스트나 딕셔너리를 넘기면 바뀌어 있어서, 타입에 따라 전달 규칙이 다르다고 오해하기 쉽습니다. 그러나 전달 규칙은 같습니다. 정수와 문자열은 불변 객체라 내부 상태를 바꿀 방법이 없고, 리스트와 딕셔너리는 변경 가능한 객체라 상태를 바꿀 수 있을 뿐입니다. 위 예제에서 ``x, y = y, x``가 호출한 쪽의 ``a``를 바꾸지 못한 것도 int가 불변이어서가 아니라 지역 이름을 다시 할당했기 때문입니다. 파라미터 ``x``에 리스트를 넘겨도 ``x = [1]``처럼 다시 할당하면 호출한 쪽은 그대로입니다.
...
FAQ의 'C 계열의 모든 언어'라는 표현은 Go의 규칙을 강조하려는 것입니다. 이 글에서 살펴본 {cpp}과 C#은 C 계열이지만 참조 파라미터를 따로 지원합니다.
...
같은 FAQ는 맵과 슬라이스 값이 내부 데이터를 가리키는 포인터를 담은 서술자(descriptor)라서 포인터처럼 동작한다고 설명합니다. 맵이나 슬라이스 값을 복사해도 그 값이 가리키는 데이터는 복사되지 않습니다. 함수 안에서 파라미터로 받은 맵에 키를 추가하면 호출한 쪽 맵에서도 그 키를 읽을 수 있지만, 파라미터에 새 맵을 대입해도 호출한 쪽의 변수는 그대로입니다. Java에서 객체의 참조값을 전달할 때와 동일한 개념입니다. 함수 안에서 슬라이스에 ``append``한 결과가 호출한 쪽에 보이지 않는 것도 같은 원리입니다. ``append``는 넘겨받은 슬라이스 값을 바꾸지 않고 길이가 늘어난 새 슬라이스 값을 반환하므로, 그 결과를 파라미터에 대입하는 것은 파라미터에 새 맵을 대입하는 것과 같습니다. 호출한 쪽에 반영하려면 반환해서 다시 대입해야 합니다.

[source,go]
.Go 슬라이스 파라미터의 요소 변경과 append
----
func modify(s []int) {
    s[0] = 100       // <1>
    s = append(s, 4) // <2>
...
func appendFour(s []int) []int {
    return append(s, 4)
}

nums := []int{1, 2, 3}
modify(nums)
fmt.Println(nums, len(nums)) // [100 2 3] 3

nums = appendFour(nums) // <3>
fmt.Println(nums, len(nums)) // [100 2 3 4] 4
----
<1> 파라미터 ``s``는 호출한 쪽의 ``nums``와 같은 배열을 가리키므로, 요소를 바꾸면 ``nums``에서도 100이 보입니다.
<2> ``append``는 길이가 4인 새 슬라이스 값을 돌려주고 그 값은 지역 변수 ``s``에만 대입됩니다. 호출한 쪽의 ``nums``는 길이가 3인 원래 슬라이스 값 그대로입니다. 용량이 남아 있어 새 요소가 같은 배열에 쓰이더라도 ``nums``의 길이는 바뀌지 않으므로, ``nums``로는 그 요소를 읽을 수 없습니다.
<3> 호출한 쪽에 반영하려면 ``append``한 슬라이스를 반환해서 다시 대입합니다. 내장 함수 ``append``가 결과를 반환하도록 설계된 것도 이 때문입니다.
...
그런데 https://ada-lang.io/docs/arm/AA-6/AA-6.2/[Ada Reference Manual 6.2]는 복사로 전달할지 참조로 전달할지를 모드가 아니라 타입으로 정합니다. ``Integer`` 같은 elementary type은 by-copy type이라 복사로 전달하고, tagged type 등 by-reference type은 참조로 전달하며, ``aliased``로 선언한 형식 파라미터는 타입과 관계없이 참조로 전달합니다. 복사로 전달된 `in out`, `out` 파라미터는 https://ada-lang.io/docs/arm/AA-6/AA-6.4[6.4.1]에 따라 서브프로그램이 정상 종료할 때 그 값을 실제 인자에 되돌려 씁니다. 이것이 정의 절에서 언급한 call by copy-restore입니다. 호출한 쪽의 변수가 바뀐다는 결과는 call by reference와 같지만 실제 전달은 복사입니다.
...
|C#|예. `ref`, `out` 파라미터에 한함. `in`, `ref readonly`도 참조로 전달하지만 대입은 불가|기본은 call by value. class 인스턴스는 참조의 복사본 전달|`ref`, `out` 파라미터 선언. 호출 지점에도 `ref`, `out` 표기
...
|Ada|부분적으로. `in out`, `out` 파라미터 중 by-reference 타입이거나 ``aliased``로 선언한 것은 call by reference. by-copy 타입은 정상 종료 시 파라미터 값을 되돌려 씀(copy-restore)|타입에 따라 by-copy 또는 by-reference|`in out`, `out` 모드 선언
|===
