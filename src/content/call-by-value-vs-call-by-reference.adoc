...
**Compilers: Principles, Techniques, and Tools** 2판은 1.6.6절에서 두 방식을 다음과 같이 설명합니다.

[quote, "Alfred V. Aho, Monica S. Lam, Ravi Sethi, Jeffrey D. Ullman", "Compilers: Principles, Techniques, and Tools 2판 1.6.6절"]
____
...
Call by value에서는 실제 파라미터가 표현식이면 평가하고, 변수이면 복사한다. 그 값은 호출된 프로시저에서 대응하는 형식 파라미터의 저장 위치에 놓인다. 이 방식은 C와 Java에서 쓰이며, {cpp}와 대부분의 다른 언어에서도 흔히 쓰는 선택지다. ...

Call by reference에서는 실제 파라미터의 주소가 대응하는 형식 파라미터의 값으로 호출된 쪽에 전달된다. 호출된 쪽의 코드에서 형식 파라미터를 쓰는 부분은 이 포인터를 따라 호출한 쪽이 가리킨 위치에 접근하도록 구현된다. 따라서 형식 파라미터의 변경은 실제 파라미터의 변경으로 나타난다.
____

인용문의 실제 파라미터(actual parameter)는 이 글의 인자에, 형식 파라미터(formal parameter)는 이 글의 파라미터에 해당합니다. 이 책은 1977년의 전신 **Principles of Compiler Design**부터 대학 컴파일러 강의의 기본 교재로 쓰여 온 표준 교과서입니다. 표지 그림 때문에 Dragon Book으로도 불립니다.

.Compilers: Principles, Techniques, and Tools 2판 표지 (이미지 출처: https://openlibrary.org/isbn/9780321486813[Open Library])
image::img/call-by-value/dragon-book-2nd.jpg["Compilers: Principles, Techniques, and Tools 2판 표지",width=200]

이 구분은 Christopher Strachey의 1967년 강의로 거슬러 올라갑니다. Strachey는 강의를 정리한 https://www.cs.cmu.edu/~crary/819-f09/Strachey67.pdf[Fundamental Concepts in Programming Languages] 3.4.2절에서 파라미터에 표현식의 R-value를 넘기는 방식을 call by value, L-value를 넘기는 방식을 call by reference라고 불렀습니다.

[quote, Christopher Strachey, Fundamental Concepts in Programming Languages 3.4.2절]
____
...
함수를 사용할(호출하거나 적용할) 때는 f[ε]라고 쓰며, ε는 표현식일 수 있다. 참조 투명한 언어라면 f[ε]를 평가하는 데 표현식 ε에 대해 알아야 할 것은 그 값뿐이다. 그러나 값에는 두 종류가 있으므로 함수 f에 ε의 R-value를 넘길지 L-value를 넘길지 정해야 한다. 어느 쪽도 가능하므로, 함수의 각 속박 변수(형식 파라미터라고도 한다)가 R-value를 요구하는지 L-value를 요구하는지 정하는 일이 함수 정의의 일부가 된다. 이 두 가지를 파라미터를 값(R-value)으로 호출하는 방식과 참조(L-value)로 호출하는 방식이라고도 부른다.
____

L-value는 대입문의 왼쪽(Left)에 놓인 표현식이 뜻하는 저장 위치이고, R-value는 오른쪽(Right)에 놓인 표현식이 뜻하는 그 위치에 담긴 값입니다. 이 강의록은 L-value와 R-value, first-class 객체 같은 용어의 출처로 지금도 인용되는 프로그래밍 언어론의 고전입니다.

두 방식을 구분하는 실용적인 기준은 한 문장으로 요약할 수 있습니다. *함수 안에서 파라미터에 새 값을 대입했을 때 호출한 쪽의 변수가 바뀌는가?* 바뀌면 call by reference이고, 바뀌지 않으면 call by value입니다. 이 기준은 이 글처럼 두 방식만 비교하고, 다시 대입할 수 있는 변수를 인자로 넘기며, 파라미터에도 대입할 수 있다고 가정할 때 성립합니다. Call by copy-restore나 call by name 등 다른 평가 전략까지 포함하면 이 질문만으로 모든 인자 전달 방식을 분류할 수는 없습니다.

또한 이 기준에서 말하는 대입은 ``targetTv = new Tv()``처럼 파라미터 변수 자체에 다른 값을 넣는 것입니다. 파라미터가 객체를 가리킨다면 다른 객체를 가리키게 바꾸는 셈입니다. ``targetTv.setChannel("Rock")``처럼 파라미터가 가리키는 객체의 내부 상태를 바꾸는 것은 별개의 문제입니다. 뒤에서 볼 오해는 이 둘을 혼동하는 것에서 생깁니다.

=== Pass by ~와 call by ~ 용어

Call by value와 call by reference는 pass by value, pass by reference라고도 부릅니다. ``call by ~``는 1960년에 발표된 ALGOL 60 보고서부터 쓰인 형태입니다. https://www.masswerk.at/algol60/report.htm[1963년 개정 보고서]의 4.7.3절이 파라미터 전달 방식을 규정하며 4.7.3.1에 'Value assignment (call by value)', 4.7.3.2에 'Name replacement (call by name)'이라는 제목을 붙였습니다. 실무와 교재에서는 ``pass by ~``도 널리 쓰입니다. 이 글에서는 ``call by ~``로 통일했습니다.
...
<3> 메서드 안에서 파라미터 ``x``와 ``y``에 서로의 값을 대입해도, 호출한 쪽의 ``x1``, ``y1``, ``x2``, ``y2``는 바뀌지 않습니다.
...
----
<1> 앞의 예제와 같이 ``myTv``에 담긴 참조값이 복사되어 changeState 메서드의 파라미터 ``targetTv``에 대입됩니다. 두 변수는 같은 객체를 가리킵니다.
<2> 파라미터 ``targetTv``를 재할당하지 않고, 파라미터가 가리키는 객체의 채널을 ``Rock``으로 바꿉니다. main 메서드의 ``myTv``가 가리키는 바로 그 객체입니다.
...
호출자 변수로 읽어도 바뀐 상태가 보이니 "객체는 call by reference로 전달된다"는 설명이 그럴듯하게 들립니다.
그러나 앞에서 설명했듯이, 두 방식을 구분하는 실용적인 기준은 파라미터에 대입한 값이 호출자 변수에 반영되는가입니다.
호출자 변수로 바뀐 상태가 보이는 것은 두 변수가 같은 객체를 가리키기 때문일 뿐이고, 전달되는 것은 여전히 참조값의 복사본입니다. 즉 call by value입니다.

**Head First Java**에서는 객체 참조를 TV 리모컨에 비유해서 설명합니다. 객체를 전달하면 TV(객체)가 복사되는 것이 아니라 그 TV를 조작할 수 있는 리모컨(참조값)이 복사됩니다. 복사된 리모컨으로 채널을 바꾸면(객체 상태 변경) 원래 리모컨을 가진 사람에게도 바뀐 채널이 보입니다. 복사된 리모컨을 새 TV에 연결하면(재할당, `targetTv = new Tv()`) 그 리모컨으로는 원래 TV를 조작할 수 없고, 원래 리모컨은 여전히 원래 TV에 연결되어 있습니다.

=== Java의 reference와 용어의 혼란

Java의 창시자 James Gosling은 공저한 **The Java Programming Language**에서 이 오해를 직접 지적했습니다.
...
.The Java Programming Language 4판 표지 (이미지 출처: https://openlibrary.org/isbn/9780321349804[Open Library])
image::img/call-by-value/java-programming-language-4th.jpg["The Java Programming Language 4판 표지",width=200]
...
앞에서 인용한 Dragon Book도 같은 절에서 Java는 call by value만 쓴다고 밝힙니다. 그런데 이어지는 call by reference 항목에서는 Java가 기본 타입이 아닌 값에는 call by reference를 쓰는 것처럼 동작한다고 적었습니다.
...
Java는 call by value만 쓰지만, 호출되는 프로시저에 객체의 이름을 넘기면 그 프로시저가 받는 값은 사실상 그 객체를 가리키는 포인터다. 따라서 호출된 프로시저는 객체 자체의 값에 영향을 줄 수 있다. ...

Call by value를 다루면서 언급했듯이, Java 같은 언어는 배열, 문자열, 그 밖의 객체를 넘기는 문제를 그 객체에 대한 참조만 복사하는 방식으로 해결한다. 그 결과 Java는 정수나 실수 같은 기본 타입이 아닌 모든 것에 call by reference를 쓰는 것처럼 동작한다.
____

'call by reference를 쓰는 것처럼 동작한다(behaves as if)'는 호출된 메서드에서 바꾼 객체 상태가 호출한 쪽에서도 보인다는 효과를 가리킵니다. 이 책이 앞에서 밝혔듯이 실제 전달 방식은 call by reference가 아니라는 전제를 깐 표현입니다. 표준 교과서도 효과를 설명할 때 'call by reference'라는 말을 빌려 쓸 만큼 두 개념은 섞이기 쉽습니다.

이런 혼란은 'reference'라는 용어에서 비롯된 것으로 보입니다. Java에서 객체를 가리키는 값을 'reference'라고 부르는데, 이 단어가 'call by reference'의 reference와 겹치면서 오해가 생긴 것입니다. 더 정확하게 표현하면 *Java는 객체를 가리키는 참조값을 call by value로 전달합니다.*
...
이와 관련된 JLS(Java Language Specification)의 서술을 보겠습니다. JLS도 '포인터'라는 단어를 씁니다. https://docs.oracle.com/javase/specs/jls/se26/html/jls-4.html#jls-4.3.1[4.3.1. Objects] 절은 참조값을 객체를 가리키는 포인터로 정의합니다.
...
앞의 ``reassign`` 예제에서 ``myTv``의 참조값이 파라미터 ``targetTv``로 복사되는 과정을 이 구조에 맞춰 따라가 보겠습니다. 그러려면 먼저 다음 개념을 알아야 합니다.
...
<2> 파라미터가 가리키는 객체의 속성을 바꾸므로 호출한 쪽에서도 바뀐 값이 보입니다. 컴파일 오류가 나는 ``reassign`` 함수를 지우고 실행하면 ``Rock``이 출력됩니다.
...
객체를 전달할 때 함수 안에서 재할당하거나 상태를 바꾼 결과도 Java와 같습니다.
...
<2> 파라미터가 가리키는 객체의 프로퍼티를 바꾸면 호출한 쪽의 ``myTv``에서도 바뀐 값이 보입니다.
...
TypeScript도 인자 전달 방식은 JavaScript와 같습니다. https://www.typescriptlang.org/docs/handbook/typescript-from-scratch.html#runtime-behavior[TypeScript 핸드북]은 원칙적으로 TypeScript는 JavaScript 코드의 런타임 동작을 바꾸지 않는다고 밝힙니다.
...
TypeScript 컴파일러는 타입 검사가 끝나면 타입 정보를 지우고 평범한 JavaScript를 내놓으므로 인자 전달 방식도 위에서 설명한 JavaScript와 동일합니다. 뒤에서 살펴볼 C#의 `ref`, `out`에 해당하는 참조 파라미터 문법도 없습니다.

=== JavaScript: The Definitive Guide의 call by reference 서술

일부만 읽으면 JavaScript의 함수 호출을 call by reference로 오해하기 쉬운 자료도 있습니다. David Flanagan이 쓰고 오랫동안 JavaScript의 표준적인 책으로 꼽혀온 https://docstore.mik.ua/orelly/webprog/jscript/ch11_02.htm[**JavaScript: The Definitive Guide** 4판]은 11.2절 'By Value Versus by Reference'의 요약 표에서 객체가 by reference로 복사·전달·비교된다고 정리합니다. 이 요약 표만 보면 JavaScript는 call by reference라고 설명해도 될 것처럼 보입니다.

그런데 같은 절의 동작 설명을 읽어 보면, 이 책이 말하는 by reference는 이 글의 앞부분에서 정의한 call by reference와 다른 의미입니다. '객체 값 전체가 복사되는 대신 객체를 가리키는 참조가 전달된다'는 의미로 쓴 표현이고, 파라미터를 재할당해도 호출한 쪽 변수에 반영되지 않는다는 점은 이 책도 똑같이 설명합니다.
...
이 설명에 이어지는 예제의 제목도 'References themselves are passed by value', 즉 참조 자체는 값으로 전달된다는 것입니다. 결국 관찰되는 동작을 설명하는 내용은 이 글과 같고, 전달되는 것을 무엇이라고 부르는지만 다릅니다.

== C

C에는 {cpp}의 참조 파라미터 같은 문법이 없으므로 call by value만 있습니다. C11 표준 초안 https://www.open-std.org/jtc1/sc22/wg14/www/docs/n1570.pdf[N1570]의 6.5.2.2 Function calls 4항은 함수 호출을 준비할 때 인자를 평가하고 각 파라미터에 대응하는 인자의 값을 대입한다고 규정합니다.
...
{cpp}는 이 글의 정의에 맞는 call by reference인 참조 파라미터를 제공합니다.
...
----
<1> ``Replace(Tv targetTv)``처럼 `ref` 없이 선언하고 ``Replace(myTv)``로 호출하면 Java의 ``reassign(myTv)``와 같아서 ``Classic``이 출력됩니다. 선언에 `ref`가 있는 상태에서 호출 지점의 `ref`만 빼면 컴파일 오류입니다.
...
Python에도 Java처럼 call by value만 있습니다. 공식 튜토리얼 https://docs.python.org/3/tutorial/controlflow.html#defining-functions[4.8 Defining Functions]는 인자가 호출된 함수의 지역 심벌 테이블에 도입되므로 값이 항상 객체 참조인 call by value로 전달된다고 설명합니다.
...
<3> ``targetTv``가 가리키는 객체의 속성을 바꾸므로 호출한 쪽의 ``my_tv``로 접근해도 바뀌어 있고, ``Rock``이 출력됩니다.

Java의 `Tv` 예제, JavaScript의 ``reassign``과 `changeState` 예제와 동일한 결과입니다.

정수나 문자열을 넘기면 호출한 쪽 변수가 그대로인데 리스트나 딕셔너리를 넘기면 바뀌어 있어서, 타입에 따라 전달 규칙이 다르다고 오해하기 쉽습니다. 전달 규칙은 같습니다. 정수와 문자열은 불변 객체라 내부 상태를 바꿀 방법이 없을 뿐입니다. 리스트를 넘겨도 함수 안에서 파라미터를 ``x = [1]``처럼 다시 묶으면 호출한 쪽은 그대로입니다.
...
다만 메서드 호출에서는 호출 지점에 ``&``가 없어도 됩니다. https://go.dev/ref/spec#Calls[Go 사양의 Calls 절]은 ``x``가 주소를 구할 수 있는 값이고 ``&x``의 메서드 집합에 ``m``이 있으면 ``x.m()``이 ``(&x).m()``의 축약 표기라고 규정합니다. 포인터 수신자 메서드는 이 규칙에 따라 컴파일러가 주소를 대신 구해 넘기므로, 호출식의 모양만으로 호출한 쪽 변수가 바뀌는지 판단할 수는 없습니다. 이때도 전달되는 것은 포인터 값입니다.

== Rust

Rust는 이 글에서 정의한 의미의 call by reference, 즉 파라미터가 호출자 변수의 별칭이 되는 전달 방식을 지원하지 않습니다. https://doc.rust-lang.org/rust-by-example/scope/borrow.html[Rust by Example]은 ``&T``를 넘기는 것을 'passed by reference'라고 부릅니다. 그러나 함수 호출이 호출자 변수의 별칭을 만들지는 않으며, ``&mut T`` 같은 참조도 하나의 값으로서 call by value로 전달됩니다. https://doc.rust-lang.org/book/ch04-01-what-is-ownership.html#ownership-and-functions[Rust Book]은 변수를 함수에 넘기면 대입과 마찬가지로 값이 이동하거나 복사된다고 설명합니다.
...
표준 라이브러리의 https://doc.rust-lang.org/std/mem/fn.swap.html[`std::mem::swap`] 함수도 `fn swap<T>(x: &mut T, y: &mut T)` 시그니처로 같은 방식을 씁니다. 가변 참조를 넘겨도 파라미터 자체를 재할당해서 호출자의 변수 바인딩을 바꾸는, 이 글에서 정의한 call by reference는 아닙니다. ``x``를 재할당하려면 파라미터를 ``mut x: &mut i32``로 선언해야 하고, 그렇게 재할당해도 호출한 쪽의 ``a``에는 아무 영향이 없습니다.
...
정의 절에서 '파라미터에 새 값을 대입했을 때 호출한 쪽의 변수가 바뀌는가'라는 기준만으로는 모든 전달 방식을 분류할 수 없다고 했습니다. Ada가 그 예입니다. Ada는 파라미터에 `in`, `in out`, `out` 모드를 선언하고, ``in out``이나 ``out`` 파라미터에 대입한 값은 호출한 쪽 변수에 반영됩니다. 호출한 쪽에서 관찰되는 결과만 보면 {cpp}의 참조 파라미터와 같습니다.
...
* 객체를 공유하는 이 동작을 call by sharing이라고 부르기도 합니다. **JavaScript: The Definitive Guide** 4판처럼 일부 자료는 객체를 공유하는 동작을 by reference라고 표현하지만, 호출자 변수의 별칭을 전달한다는 의미의 call by reference와는 구분해야 합니다.
