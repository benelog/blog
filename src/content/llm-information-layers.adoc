...
최근 몇 년 사이에 그 중간에 위치하는 기법들도 늘었습니다.

.LLM에 정보를 전달하는 계층 분류
image::img/llm-information-layers/mindmap.drawio.png[LLM에 정보를 전달하는 계층 마인드맵,align="center"]
...
* **선호 학습 (preference learning)**: "어떤 응답이 더 나은가"를 가중치에 새깁니다. 지식이 아니라 **가치/취향** 주입이라는 점에서 결이 다릅니다.
...
* **RLVR(Reinforcement Learning with Verifiable Rewards)**: 수학 정답, 코드 실행 결과처럼 자동 채점 가능한 신호로 강화학습. DeepSeek-R1 계열은 이 방식이고, o1도 강화학습 기반 추론 모델로 알려져 있지만 보상 설계의 세부는 공개되지 않았습니다. 사람의 선호가 아니라 **풀이 능력 자체**를 새긴다는 점에서 RLHF와 구분됩니다. Cursor의 Composer 2.5는 이를 에이전트 환경으로 확장한 예로, 도구(편집·검색·테스트 실행)를 호출하는 멀티스텝 롤아웃에서 **테스트 통과 여부**를 검증 가능한 보상으로 씁니다.
...
공통점은 **inference time에 토큰으로 입력된다**는 것입니다. 모델은 최종적으로 토큰과 role/구조 신호를 보고 처리하며, 외부 데이터의 원천 신뢰도를 직접 검증하지는 않습니다.
...
** LoRA(Low-Rank Adaptation) / QLoRA(Quantized LoRA): 파인튜닝으로 생기는 가중치 변화(ΔW)를 두 작은 행렬의 곱(B·A)인 저랭크(low-rank) 행렬로 근사해, 원래 가중치는 동결하고 이 작은 행렬만 학습. 가중치 변화가 실은 소수의 지배적인 방향만으로 충분히 표현된다(intrinsic rank가 낮다)는 가정에 기댑니다. 고차원 데이터를 분산이 큰 몇 개의 주성분으로 압축하는 **PCA/SVD의 저랭크 근사와 같은 발상**으로, 다만 ΔW를 SVD로 직접 분해하는 대신 두 행렬을 경사하강법으로 학습한다는 점이 다릅니다. QLoRA는 여기에 베이스 모델 양자화(정밀도 축소)를 더해, **랭크 축소(LoRA)와 정밀도 축소(양자화)라는 직교하는 두 압축 축을 함께 쓰는 셈**입니다
...
* **Decoding 제어**: 출력 확률 분포를 직접 조작. "주입"보다 **출력 제한**에 가깝지만, 도메인 지식을 grammar로 강제하면 사실상 정보 주입입니다.
...
**아키텍처 레이어**:
모델 구조 자체. MoE(Mixture of Experts)로 전문가를 분리하거나, 검색 모듈을 구조에 통합하거나, 메모리 토큰 슬롯을 두는 식.
가중치 학습 이전에 "**어떤 형태로 정보를 담을 것인가**"를 정합니다. retrieval-augmented architecture/pretraining의 예:
