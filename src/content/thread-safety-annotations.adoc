...
주의할 점은 IntelliJ IDEA 2026.2의 link:{intellij-source-base}/java/java-backend/resources/META-INF/Inspections.xml[인스펙션 등록 정보]에서 'Concurrency annotation issues' 그룹이 모두 `enabledByDefault="false"`, ``level="WARNING"``으로 선언되어 있다는 것입니다. Settings의 Editor | Inspections | Java | Concurrency annotation issues에서 직접 켜야 합니다. 켜도 편집기 안의 경고라서 Error Prone처럼 `javac` 컴파일을 막지는 못합니다. CI에서 강제하려면 Qodana 같은 별도 실행 환경이 필요합니다. Qodana는 같은 인스펙션을 제공하므로 https://www.jetbrains.com/help/qodana/inspection-profiles.html[Qodana 프로필]에서 검사를 켜고 https://www.jetbrains.com/help/qodana/quality-gate.html[실패 조건]을 설정하면 됩니다.
