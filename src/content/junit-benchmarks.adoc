...
https://github.com/carrotsearch/junit-benchmarks[junit-benchmarks]는 Carrot Search에서 만든 JUnit 4 확장입니다. 평범한 JUnit 테스트 메소드에 애노테이션만 붙여서 벤치마크로 바꿔줍니다.

* `@BenchmarkOptions` : 반복 횟수(`benchmarkRounds`), 워밍업 횟수(`warmupRounds`), 동시 실행 스레드 수(`concurrency`)를 지정합니다.
* `@BenchmarkMethodChart` : 메소드별 실행 시간을 HTML 차트로 생성합니다. 결과를 H2 DB에 누적해서 차트를 그리는 방식이라, 예제의 link:{source-repo}#file-pom-xml[pom.xml]에 `h2` 의존성이 들어 있는 이유이기도 합니다.
