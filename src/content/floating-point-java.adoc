= IEEE 754 부동소수점 오차와 Java의 대안
정상혁
2026-08-15
:jbake-type: post
:jbake-status: published
:jbake-tags: java, code-review
:description: 자바의 double에 0.1을 저장할 때 IEEE 754 가수 52비트에서 반올림 오차가 생기는 원리를 설명하고, 그 오차가 장애로 이어진 NEIS 성적 오류와 패트리어트 미사일 사례를 살펴봅니다. 오차를 피하는 BigDecimal과 정수형 최소 단위의 선택 기준, Java Money 같은 도메인 특화 라이브러리도 정리합니다.
...
jshell은 JDK 9부터 포함된 대화형 실행 도구입니다.
...
jshell 명령행에서 0.1로 입력했지만 저장된 값은 0.1보다 약 5.55 × 10^-18^만큼 큰 수입니다.

이 오차는 자바만의 문제가 아닙니다.
JavaScript의 `number` 타입도 IEEE 754 binary64 형식을 쓰기 때문에, Chrome 개발자 도구 콘솔에서 같은 식을 입력하면 결과가 똑같습니다.

image::img/floating-point-java/chrome-console-float.png[Chrome 개발자 도구 콘솔에서 확인한 부동소수점 오차,width=800]
...
1985년에 나온 첫 표준인 IEEE 754-1985에서는 이 두 형식을 single, double이라고 불렀습니다.
2008년 개정판에서 이름이 binary32, binary64로 바뀌었고, 이 이름이 현행 표준인 IEEE 754-2019까지 이어집니다.
...
* 지수(e)는 실제 지수에 1023을 더한 값을 11비트에 저장합니다. 이렇게 하면 -1022~1023의 지수를 1~2046의 부호 없는 정수로 저장할 수 있고, 값을 읽을 때는 거꾸로 1023을 뺍니다.
...
image::img/floating-point-java/ieee754-double.png[IEEE 754 double의 비트 구조와 0.1 저장 예,width=800]
...
문제는 가수입니다.
0.6을 2진수로 바꾸면 0.1001 1001 1001…처럼 1001이 끝없이 반복됩니다.
52비트 안에 담으려면 반올림할 수밖에 없습니다.
52비트 뒤에 이어지는 비트가 1로 시작하므로 올림이 일어나, 가수의 마지막 네 비트 1001이 1010이 됩니다.
그 결과 0.1이 아니라 0.1보다 아주 조금 큰 수가 저장됩니다.
...
반면 0.1 = 1/10처럼 기약분수로 나타냈을 때 분모에 5가 들어 있는 십진 소수는 2진수로는 무한히 반복되는 소수가 되어 반올림 오차를 피할 수 없습니다.
표준을 더 자세히 알고 싶다면 https://ko.wikipedia.org/wiki/IEEE_754[IEEE 754 위키백과 문서]를 참고할 수 있습니다.

== 2진수 표현 오차로 생긴 장애 사례

과학 실험의 데이터처럼 근삿값을 다루는 영역에서는 이런 오차가 허용 범위 안에 드는 경우가 많습니다.
그러나 금액, 성적처럼 값이 정확히 같아야 하거나, 오차가 누적되는 계산 로직에서는 작은 오차도 치명적인 버그가 됩니다.
이런 오차가 실제 장애로 이어진 국내와 해외 사례를 하나씩 살펴보겠습니다.

=== 2011년 NEIS 성적 처리 오류

2011년 7월 차세대 교육행정정보시스템(NEIS·나이스)에서 성적 처리 오류가 발생해, 전국 823개 고교에서 2만 9천여 명의 학기말 석차가 바뀌었습니다.
대입 수시모집을 앞둔 시점이어서 파장이 컸습니다.

나이스는 지필고사와 수행평가 점수를 합해 한 학기 총점을 산출했습니다.
총점이 같은 학생들의 석차는 학교가 45가지 방법 중에서 고른 기준으로 정했습니다(https://m.yeongnam.com/view.php?key=20110726.010061333590001[영남일보 기사]).
예를 들어 지필고사 점수를 우선하는 학교라면, 지필고사 65점에 수행평가 25점을 받은 A 학생과 지필고사 60점에 수행평가 30점을 받은 B 학생은 총점이 똑같이 90점이어도 A 학생이 앞선 등수를 받습니다.
동점자를 구분할 기준을 학교마다 미리 정해 두어야 할 만큼, 동점 판별은 석차 산출에서 중요한 절차입니다.

나이스는 총점을 소수점 이하 16자리까지 표시하도록 짜여 있었는데, 그 소수점 이하 자리에서 불규칙적으로 '1'이 나타나는 값이 발생했습니다.
글 앞부분에서 1.03 - 0.42의 결과로 0.6100000000000001이 나온 것과 같은 형태입니다.
90과 90.00000000000001은 같은 값이 아니므로, 동점 여부를 그대로 비교하면 두 학생은 동점자가 아니고 오차가 붙은 쪽이 더 높은 점수로 정렬됩니다.
이 때문에 총점이 같아야 할 학생이 동점자로 판별되지 않거나 동점자 사이의 석차가 뒤집혔습니다.
(https://use.go.kr/news/user/bbs/BD_selectBbs.do?q_bbsSn=1005&q_bbsDocNo=13244[울산광역시교육청 안내문]).
교육과학기술부는 이 계산 오차를 보정하지 않은 것이 버그의 원인이라고 설명했습니다(https://www.khan.co.kr/article/201107222144165[경향신문 기사]).

교육과학기술부 집계로는 전국 823개 고교에서 2만 9,007명의 석차가 바뀌었고, 그중 350개교 2,416명은 석차 등급까지 바뀌었습니다(https://www.jejunews.com/news/articleView.html?idxno=957280[제주일보 기사]).
학교들은 오류 정정 프로그램으로 성적을 다시 계산하고 성적표를 재발송해야 했습니다(경향신문 기사).

교육과학기술부 특별점검단이 같은 해 9월에 발표한 점검 결과에는 어떤 타입에서 오차가 났는지까지 나옵니다.
구 나이스의 프로그램을 차세대 나이스로 재개발하는 과정에서 "새로 설치된 데이터베이스(DB2)의 특성상 나타나는 실수형(Double형) 자료 연산 오류를 예측하지 못하여 성적 처리 오류가 발생했던 것을 확인했다"는 내용입니다(https://www.etnews.com/201109030016[전자신문 기사]).
즉 성적은 DB2의 DOUBLE 타입으로 계산되었습니다.
DB2의 DOUBLE 타입은 IEEE 754 binary64를 따르므로 자바의 ``double``과 같은 형식입니다.
그리고 그 오차를 바로잡는 '오류 보정 코드'가 일부 프로그램에 빠져 있어 그곳에서 석차가 뒤바뀐 것입니다(https://www.yna.co.kr/view/AKR20110902079600004[연합뉴스 기사]).

당시 네트워크 개발자 이경문 씨는 https://www.dailysecu.com/news/articleView.html?idxno=319[데일리시큐 기사]에서 "정확한 수치 표현을 해야 하는 곳에서 부동소수점을 이용한 것"이 문제라며, 오차를 보정할 것이 아니라 정수로 처리하도록 설계를 바꾸거나 무한대의 정밀도를 지원하는 타입을 써야 한다고 지적했습니다.
이 글 뒷부분에서 소개하는 정수형 최소 단위와 ``BigDecimal``이 바로 그 두 방안입니다.
이 사건은 성적처럼 소수점 계산이 들어가는 값을 부동소수점으로 다루면 동점 여부 판정 같은 비교 연산에서 오류가 날 수 있음을 보여 줍니다.
...
미사일을 방어하는 패트리어트 시스템은 0.1초 단위로 시간을 세어 목표물이 나타날 구간을 예측하는데, 센 횟수를 초로 바꿀 때 24비트 고정소수점 레지스터에 담은 0.1을 곱했습니다.
0.1은 2진수로는 무한히 반복되는 소수이므로 24비트 이후는 잘려 나갔고, 0.1초가 지날 때마다 약 0.000000095초의 오차가 쌓였습니다.
연속 가동 20시간이면 시계가 0.0687초, 사건 당시 포대처럼 100시간이면 실제보다 0.3433초 뒤처집니다.
초속 2km로 날아오는 스커드 미사일이라면 0.3433초 동안 687m를 이동합니다.
추적 레이더는 실제 미사일 위치에서 687m 벗어난 구간을 탐색했습니다.
레이더가 그 구간에서 목표물을 찾지 못해 요격 미사일은 발사되지 않았고, 스커드 미사일은 병영에 떨어졌습니다.
...
``double``형 연산의 오차를 피하는 대안으로 ``BigDecimal``을 쓰는 방법과 금액을 최소 단위의 정수로 다루는 방법을 소개합니다.
...
주의할 점이 하나 있습니다. 생성자에 ``double``을 넘기면 십진 소수를 ``double``에 담을 때 생긴 반올림 오차가 ``BigDecimal``에 그대로 전달됩니다.
앞에서 봤듯이 ``new BigDecimal(0.1)``은 0.1이 아니라 ``double``에 저장된 근삿값을 옮겨 담습니다.
이 오차를 피하려면 문자열을 받는 생성자나 ``BigDecimal.valueOf()``를 사용해야 합니다.
``BigDecimal.valueOf(0.1)``은 ``Double.toString(0.1)``이 돌려주는 문자열 "0.1"로 값을 만들므로 0.1이 됩니다.
...
여기서 쓴 ``java.math.RoundingMode``는 0.5를 올릴지(`HALF_UP`), 가장 가까운 짝수로 보낼지(`HALF_EVEN`), 무조건 버릴지(`DOWN`) 같은 반올림 방식을 고르는 열거형입니다.
예를 들어 2.5와 3.5를 정수로 만들면 ``HALF_UP``은 3과 4, ``HALF_EVEN``은 2와 4, ``DOWN``은 2와 3이 됩니다.
...
이 항목은 ``BigDecimal``과 함께 ``int``, ``long``도 대안으로 제시하는데, 둘 중 무엇을 고를지는 뒤의 선택 기준 절에서 정리합니다.

=== 정수로 계산하기

금액을 원(₩)이나 센트 같은 최소 단위의 정수로 다루는 방법도 널리 쓰입니다.
특히 1원 이하의 가치가 적은 한국에서는 업무 규칙과 어긋나지 않는다면 실용적인 방법입니다.
같은 이유로 기존 시스템의 데이터베이스도 원화 금액을 정수 타입 칼럼으로 정의한 경우가 많습니다.
...
반올림 처리에는 ``BigDecimal``을 쓰지만, 금액 파라미터 `amount`와 리턴 타입은 ``long``입니다.
...
`long` 값이 ``float``로 변환되면서 정밀도를 잃기 때문에, 금액 파라미터와 리턴 타입이 정수여도 금액이 16,777,216원(2^24^)을 넘으면 오차가 생길 수 있습니다.
...
100%를 곱했으니 16,777,217이 그대로 나와야 하는데 1원이 줄었습니다.
``float``는 가수 23비트에 생략된 1비트를 더해 24비트 정밀도를 가지므로 2^24^(16,777,216)을 넘는 정수를 빠짐없이 표현하지는 못하고, ``16_777_217L``은 ``float``로 변환되는 순간 16,777,216이 됩니다.
입력을 정수로 유지해도 중간 계산 한 곳이 부동소수점이면 오차를 피하지 못합니다.

인용한 예제를 고쳐서 비율까지 ``BigDecimal``로 옮기면 16,777,217이 그대로 나옵니다.
...
== BigDecimal과 정수 계산의 선택 기준

계산 정밀도가 중요한 모듈이라면 적어도 메서드 파라미터와 리턴 타입에는 ``double``과 ``float``를 쓰지 않는 것이 좋습니다.
대신 쓸 타입으로는 앞에서 본 ``BigDecimal``과 정수형 최소 단위(``long``) 두 가지가 있습니다.
...
반대로 성능이 중요하고 소수점을 직접 추적할 수 있다면 ``int``나 ``long``을 쓰라는 대안도 함께 제시하는데, 앞 절에서 본 정수로 계산하는 방법과 같습니다.
...
금융 애플리케이션의 성능 개선을 맡으면 언젠가는 ``BigDecimal``을 걷어 내게 된다는 자신의 경험도 근거로 듭니다.
``BigDecimal``이 처음부터 가장 큰 지연 원인은 아니지만, 다른 병목을 고쳐 나가다 보면 결국 가장 느린 부분으로 올라온다는 것입니다.
댓글에서는 트레이딩·금융 시스템이 ``BigDecimal``보다 먼저 있었고, 십진 소수 타입 없이 C++로 만든 시스템도 많다고 덧붙입니다.

정수형 최소 단위 계산은 시스템 자원을 적게 쓴다는 장점이 있습니다.
`long` 타입의 값은 객체를 만들지 않고, 연산도 CPU의 정수 연산으로 끝납니다.

대신 `long` 방식은 중간에 나눗셈이 있거나 ``double``이나 ``float``가 섞인 연산이 있으면 개발자가 정확성을 더 많이 의식해야 합니다.
반올림 규칙을 정해야 하는 것은 ``BigDecimal``도 마찬가지이지만, ``long`` 방식에서는 그 규칙을 빠뜨렸을 때 생기는 오류를 알아차리기가 더 어렵습니다.
``BigDecimal``은 십진수로 끝나지 않는 나눗셈에서 반올림 방식을 빠뜨리면 앞에서 본 것처럼 예외를 던지지만, `long` 나눗셈은 소수점 이하를 경고 없이 버립니다. 예를 들어 ``10 / 3``은 3이 됩니다.
또 기본 산술 연산자로 계산하므로, `calculate()` 예제의 ``amount * rate * 0.01``처럼 부동소수점이 섞여도 컴파일러가 막지 않습니다.
최소 단위를 무엇으로 볼지 시스템 전체가 같은 규칙으로 지켜야 한다는 점도 ``BigDecimal``에는 없는 부담입니다.

지금까지의 내용을 정리하면 다음과 같습니다.

[cols="1,1,2,2", options="header"]
.BigDecimal과 정수 계산 비교
|===
^|자료형 ^|저장 방식 ^|강점 ^|감수할 점
...
어떤 타입을 쓸지 정했다면, 팀 구성원 누구나 이해할 수 있는 단순한 규칙으로 문서에 적어야 합니다.
예를 들면 "금액은 모든 계층에서 ``long``(원 단위)으로 다루고, 비율 계산만 ``BigDecimal``로 한 뒤 ``RoundingMode.DOWN``으로 버린다" 같은 규칙입니다.
시스템의 담당 개발자는 계속 바뀔 수 있습니다.
모든 사람이 이 글에서 다룬 오차가 생기는 원리까지 이해하고 있다고 전제하기는 어렵습니다.
규칙이 명확하고 단순하면 새로 투입된 개발자도 같은 방식으로 따를 가능성이 높아집니다.
...
규칙을 정하고 나면 그와 어긋나는 기존 코드까지 고쳐야 앞으로도 일관성이 유지됩니다.
그런데 이미 잘 돌아가고 있는 데다 금액을 직접 다루는 코드는 고치기가 무섭습니다.
그래서 한번 자리 잡은 방식을 바꾸지 않는 경우도 많습니다.
기존 코드를 고쳐야 할 때를 대비해서라도, 금액처럼 민감한 수치를 다루는 로직은 앞의 ``calculate()``처럼 별도 메서드로 분리해 두어야 합니다.
그러면 데이터베이스나 외부 시스템 없이 입력과 기대 결과만으로 테스트할 수 있습니다.
...
https://javamoney.github.io/[Java Money]는 금액과 통화를 함께 다루는 API로, JSR 354로 표준화됐습니다.
...
서로 다른 통화의 금액을 섞어 계산하는 실수를 API가 실행 시점에 예외로 막아 줍니다.
...
Moneta가 제공하는 `MonetaryAmount` 구현체 중 대표적인 두 가지가 이 글에서 본 두 가지 수치 표현에 그대로 대응합니다.

* `Money`: 금액을 ``BigDecimal``로 저장합니다. 그만큼 앞에서 정리한 ``BigDecimal``의 비용도 그대로 집니다.
* https://github.com/JavaMoney/jsr354-ri/blob/master/moneta-core/src/main/java/org/javamoney/moneta/FastMoney.java[FastMoney]: 금액을 `long` 하나에 최소 단위로 저장합니다. 소수점 아래 5자리로 스케일이 고정되므로 표현할 수 있는 최대 금액은 약 92조(`Long.MAX_VALUE` / 10^5^)입니다. 이런 제약이 있는 대신 빠르고, 자바독은 ``Money``보다 10~15배 빠르다고 소개합니다.

인터페이스가 같으므로 속도가 문제가 되면 `Money` 대신 ``FastMoney``로 바꿔 끼울 수 있습니다.
도메인 타입을 도입하더라도 수치 표현을 무엇으로 할지는 여전히 정해야 하고, 앞 절에서 정리한 트레이드오프가 구현체 선택으로 다시 나타납니다.
...
* https://unitsofmeasurement.github.io/[Units of Measurement API (JSR 385)]: 길이나 질량 같은 물리량과 그 단위를 타입으로 표현합니다. API 패키지는 ``javax.measure``이고 참조 구현은 https://unitsofmeasurement.github.io/indriya/[Indriya]입니다. 금액에서 통화가 다르면 예외가 나는 것처럼 단위가 맞지 않는 계산을 걸러내는데, 이쪽은 컴파일 시점에 막습니다. 예를 들어 ``Quantity<Length>``에 ``Quantity<Mass>``를 더하는 코드는 제네릭 타입이 달라 컴파일되지 않습니다.
...
이때도 수를 ``BigDecimal``과 정수 중 무엇으로 담을지는 이 글에서 설명한 기준에 따라 정합니다.
그리고 중간 연산에 ``float``나 ``double``이 끼어들어 생기는 오류를 막아야 합니다.
...
* https://techblog.woowahan.com/2560/[Spock으로 테스트코드를 짜보자 - 우아한형제들 기술 블로그]
...
** https://m.yeongnam.com/view.php?key=20110726.010061333590001[나이스 프로그램에 문제 '쓰레기 값' 처리 누락 - 영남일보 (2011.07.26)]
** https://www.dailysecu.com/news/articleView.html?idxno=319[차세대 골칫거리 '나이스', 무엇이 문제였나 - 데일리시큐 (2011.07.26)]
** https://www.yna.co.kr/view/AKR20110902079600004['나이스오류' 예견된 사고…설계서ㆍ테스트 없었다 - 연합뉴스 (2011.09.02)]
** https://www.etnews.com/201109030016[교육행정정보 시스템 '나이스' 개발 부실…삼성SDS 상대 소송 고려 - 전자신문 (2011.09.03)]
* 패트리어트 미사일 요격 실패
