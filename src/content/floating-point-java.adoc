...
그리고 그 오차를 잡아 주는 '오류 보정 코드'가 일부 프로그램에 빠져 있어 그곳에서 석차가 뒤바뀐 것입니다(https://www.yna.co.kr/view/AKR20110902079600004[연합뉴스 기사]).
...
주의할 점이 하나 있습니다. 생성자에 ``double``을 넘기면 십진 소수를 ``double``에 담을 때 생긴 반올림 오차가 ``BigDecimal``에도 그대로 남습니다.
...
어떤 타입을 쓸지 정했다면, 팀 구성원 누구나 이해할 수 있는 단순한 규칙으로 정리해 문서에 남겨야 합니다.
