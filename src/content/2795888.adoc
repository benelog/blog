...
SPel이 잘 먹는지는 다음과 같이 테스트 할 수 있습니다.
...
위의 테스트 코드에서는 @ContextConfiuguration선언 뒤에 Application context 파일 위치를 지정하지 않았으므로,  default로 참조되는 같은 패키지 디렉터리의 HostNameTest-context.xml에 'localHost' bean이 선언되어 있어야 겠죠.
...
다른 방법으로는,  JavaConfig를 활용해서 직접 명시적으로 메서드를 호출해서 hostName을 주입해도 됩니다.
